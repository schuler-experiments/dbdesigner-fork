...
// Description
//   Contains the core classes like TPdfDoc, TPdfCanvas, ...
//   which include the core functionality
//
// Changes:
//   Version 1.0, 12.09.2003, Mike
//     adapted version from Takezou's PowerPDF, Version 0.9 (beta), 2001.09.13
//
//----------------------------------------------------------------------------------------------------------------------
...
//  TPdfLink = class;
  ...
  private
    ...
    FVerticalPageOriginTop: Boolean;
  ...
  public
    ...
    property VerticalPageOriginTop: Boolean read FVerticalPageOriginTop write FVerticalPageOriginTop;
  end;
  ...
  private
    ...
    function ConvertHValue(y: Single): Single;
  protected
  public
    ...
    procedure TextOutAlignRight(X, Y: Single; Text: string);
    procedure TextOutBaseLinePoint(X, Y: Single; Text: string);
    ...
    procedure DrawLine(x1, y1, x2, y2: Single; Width: Single = 1);
...
procedure TPdfDoc.SaveToFile(fName: string);
var FOutFile: TFileStream;
begin
  // create output-filestream.
  FOutFile := TFileStream.Create(fName, fmCreate);
  try
    SaveToStream(FOutFile);
  finally
    FOutFile.Free;
  end;
end;
...
begin
  ...
  FIsVirtual := false;
  FAttr.FVerticalPageOriginTop:=True;
end;
...
begin
  ...
  if FMediaBox <> nil then
    ...
    EPdfInvalidOperation.Create('Can not change width of this page..');
end;
...
function TPDFCanvas.ConvertHValue(y: Single): Single;
begin
  if(FAttr.FVerticalPageOriginTop)then
    ConvertHValue:=PageHeight-y
  else
    ConvertHValue:=y;
end;
  ...
  begin
    ...
    with FAttr do
      ...
      begin
        ...
        HorizontalScaling := FPage.PdfNumberByName('_HScaling').Value;
...
begin
  ...
  with FPage do
    begin
      ...
      AddInternalItem('_HScaling', TPdfNumber.CreateNumber(FAttr.HorizontalScaling));
...
begin
  BeginText;
  if(FAttr.FVerticalPageOriginTop)then
    MoveTextPoint(X, Y+FAttr.FontSize*0.76)
  else
    MoveTextPoint(X, Y-FAttr.FontSize*0.76);
  ShowText(Text);
  EndText;
...
procedure TPdfCanvas.TextOutAlignRight(X, Y: Single; Text: string);
begin
  TextOut(X-TextWidth(Text), Y, Text);
end;

procedure TPdfCanvas.TextOutBaseLinePoint(X, Y: Single; Text: string);
begin
  BeginText;
  MoveTextPoint(X, Y);
  ShowText(Text);
...
begin
  ...
  TextOut(ARect.Left + XPos, ARect.Top, Text);
...
begin
  ...
  GSave;
  if(FAttr.FVerticalPageOriginTop)then
    Concat(AWidth, 0, 0, AHeight, X, ConvertHValue(Y+AHeight))
  else
...
var
  S: string;
begin
  S := _FloatToStrR(x) + ' ' + _FloatToStrR(ConvertHValue(y)) + ' m'#10;
  WriteString(S);
end;
...
var
  S: string;
begin
  S := _FloatToStrR(x) + ' ' + _FloatToStrR(ConvertHValue(y)) + ' l'#10;
  WriteString(S);
end;
...
begin
  S := _FloatToStrR(x1) + ' ' +
       _FloatToStrR(ConvertHValue(y1)) + ' ' +
       _FloatToStrR(x2) + ' ' +
       _FloatToStrR(ConvertHValue(y2)) + ' ' +
       _FloatToStrR(x3) + ' ' +
       _FloatToStrR(ConvertHValue(y3)) + ' c'#10;
  WriteString(S);
end;
...
begin
  S := _FloatToStrR(x2) + ' ' +
       _FloatToStrR(ConvertHValue(y2)) + ' ' +
       _FloatToStrR(x3) + ' ' +
       _FloatToStrR(ConvertHValue(y3)) + ' v'#10;
  WriteString(S);
end;
...
begin
  S := _FloatToStrR(x1) + ' ' +
       _FloatToStrR(ConvertHValue(y1)) + ' ' +
       _FloatToStrR(x3) + ' ' +
       _FloatToStrR(ConvertHValue(y3)) + ' y'#10;
  WriteString(S);
end;
...
begin
  if(FAttr.FVerticalPageOriginTop)then
    S := _FloatToStrR(x) + ' ' +
         _FloatToStrR(ConvertHValue(y+height)) + ' ' +
         _FloatToStrR(width) + ' ' +
...
begin
  if(FAttr.FVerticalPageOriginTop)then
    S := _FloatToStrR(tx) + ' ' +
         _FloatToStrR(ConvertHValue(ty)) + ' Td'#10
  else
    S := _FloatToStrR(tx) + ' ' +
...
procedure TPdfCanvas.DrawLine(x1, y1, x2, y2: Single; Width: Single = 1);
begin
  SetLineWidth(Width);
  MoveTo(X1, Y1);
  LineTo(X2, Y2);
